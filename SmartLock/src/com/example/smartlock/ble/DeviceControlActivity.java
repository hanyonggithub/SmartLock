...
public class DeviceControlActivity extends Activity {
    ...
    private final ServiceConnection mServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder service) {
            ...
            Log.e(TAG, "mServiceConnection connected,then connect device "+mDeviceAddress);
        ...
        public void onServiceDisconnected(ComponentName componentName) {
            Log.e(TAG, "mServiceConnection disConnected");
    ...
    private final BroadcastReceiver mGattUpdateReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothLeService.ACTION_GATT_CONNECTED.equals(action)) {
                mConnected = true;
                Log.e(TAG, "mGattUpdateReceiver device connected");
                updateConnectionState(R.string.connected);
                invalidateOptionsMenu();
            } else if (BluetoothLeService.ACTION_GATT_DISCONNECTED.equals(action)) {
                mConnected = false;
                Log.e(TAG, "mGattUpdateReceiver device disconnected");
                updateConnectionState(R.string.disconnected);
                invalidateOptionsMenu();
                clearUI();
            } else if (BluetoothLeService.ACTION_GATT_SERVICES_DISCOVERED.equals(action)) {
                // Show all the supported services and characteristics on the user interface.
            	 Log.e(TAG, "mGattUpdateReceiver discover services");
                displayGattServices(mBluetoothLeService.getSupportedGattServices());
            } else if (BluetoothLeService.ACTION_DATA_AVAILABLE.equals(action)) {
            	Log.e(TAG, "读取到数据--------"+intent.getStringExtra(BluetoothLeService.EXTRA_DATA));
...
//                                        WriteBytes= hex2byte(editTextNumEditText.getText().toString().getBytes());
                                    	Log.e(TAG, "write --------");
//                                        WriteBytes= hex2byte("A361646D696EFF0000000000000f0c0f1713FF78".getBytes());
                                        WriteBytes= hex2byte("AA".getBytes());
                                    }
